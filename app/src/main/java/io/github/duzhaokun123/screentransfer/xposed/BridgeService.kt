...
object BridgeService {
    ...
    private const val TAG = "ScrTsf_Bridge"

    private var appUid = 0

    fun register(pms: IPackageManager) {
        log(TAG, "Initialize ScrTsfManager - Version ${BuildConfig.VERSION_NAME}(${BuildConfig.VERSION_CODE})")
        val service = ScrTsfManager()
...
//        }
    ...
    const val TRANSACTION = ('S'.code shl 32) or ('C'.code shl 24) or ('T'.code shl 16) or ('F'.code shl 8) or 'D'.code
                ...
                runCatching {
                    ...
                    when (data.readInt()) {
                        ACTION_GET_BINDER -> {
                            ...
                            reply?.writeStrongBinder(ScrTsfManager.instance)
                            return true
                        }
